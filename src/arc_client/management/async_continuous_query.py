"""Asynchronous continuous query client for Arc."""
...
class AsyncContinuousQueryClient:
    """Asynchronous client for continuous query management."""
    ...
    async def create(
        ...
        try:
            response = await self._http.post("/api/v1/continuous_queries", json=payload)
            data = response.json()
    ...
    async def list(
        ...
        try:
            ...
            response = await self._http.get("/api/v1/continuous_queries", params=params)
            data = response.json()
    ...
    async def get(self, query_id: int) -> ContinuousQuery:
        """Get continuous query details."""
        try:
            response = await self._http.get(f"/api/v1/continuous_queries/{query_id}")
            data = response.json()
            if not data.get("success", True):
    ...
    async def update(
        ...
        try:
            response = await self._http.put(f"/api/v1/continuous_queries/{query_id}", json=payload)
            data = response.json()
            if not data.get("success", True):
                error = data.get("error", "Unknown error")
                if "not found" in error.lower():
    ...
    async def delete(self, query_id: int) -> None:
        ...
        try:
            response = await self._http.delete(f"/api/v1/continuous_queries/{query_id}")
            data = response.json()
            if not data.get("success", True):
                error = data.get("error", "Unknown error")
                if "not found" in error.lower():
    ...
    async def get_executions(self, query_id: int, limit: int = 50) -> List[CQExecution]:
